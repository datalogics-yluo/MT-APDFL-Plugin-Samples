...
/* DLADD */
	...
	printf("Will flatten %d PDF files\n",numFiles);
...
#ifdef WIN_PLATFORM
	strcpy_safe(firstFile, 1024, "xps2pdf.xps");
	for(ASInt32 index = 0; index < loop; index++)
	{
		//Copy Input.pdf to Input-[index].pdf
		strcpy_safe(otherFile, 1024, folderName);
		strcat_safe(otherFile, 1024, "\\xps2pdf");
		char str[50];
		sprintf_safe(str, "-%d.xps", index);
		strcat_safe(otherFile, 1024, str);
		BOOL b = CopyFile(firstFile, otherFile, 0);
	}
	
#else //Linux/Mac/Unix
	strcpy(firstFile, "xps2pdf.xps");
	for(int index = 0; index < loop; index++)
	{
		//Copy xps2pdf.xps to xps2pdf-[index].xps
		strcpy(otherFile, folderName);
		strcat(otherFile, "/xps2pdf");
		char str[50];
		sprintf(str, "-%d.xps", index);
 ...
 */
	...
	for (ASInt32 i = 0; i < numThreads; i++) {
		...
		sprintf_safe(buff, 40, "MT-XPS2PDF thread %d", i);
		myThreadArgs[i].tName = buff;

		myThreadArgs[i].watchFolder = myWF;

		if (!createThread2(OneXPS2PDF, &myThreadArgs[i], myThreads[i], pattr)){	// DLADD
